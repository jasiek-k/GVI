...
const LandingSlider = () => {
  const [photosToDisplay, setPhotos] = useState(
    photosArray.filter((_item, index) => index !== 0)
  );
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (!photosToDisplay.length) {
        ...
        setPhotos(
          photosToDisplay.filter((_item, index) => index !== drawnIndex)
        );
      }
  ...
  return (
    <img
      ref={photoRef}
      src={displayedPhoto}
      className="landing-section__content--image"
      alt="Atlas of Modernity exhibition"
    />
  );
};
